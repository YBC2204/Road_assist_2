      ...
      {orderCompleted && orderIds.map((orderId, index) => (
        <div key={index} className='text-black w-[90%] bg-slate-300 flex flex-col py-3 px-5 gap-3 rounded-[22px] mb-8 mt-5 ml-4'>
          <div className='flex flex-col'>
            <p className='font-bold uppercase text-xs text-BLACK-500 text-right'>ORDER ID: {orderId}</p>
  
            <p className='font-bold uppercase text-center text-xl mt-2'>{pumpData[index]?.pump_name}</p> {/* Displaying the retrieved pump name */}
            <p className='font-bold text-center'>Company: {pumpData[index]?.Company}</p>
            <p className='font-bold text-center'>{pumpData[index]?.Address}</p>
            <div className='flex flex-col sm:flex-row justify-between pr-2 pt-1'>
              <div className='flex flex-col sm:flex-row'>
                <p className='pr-2 font-semibold'>Vehicle Name: {orderData[index]?.Vehicle_name}</p>
                <p>Car Color: {orderData[index]?.car_color}</p>
              </div>
              <div className='flex flex-col sm:flex-row'>
                <p className='pr-2'>Fuel Amount: {orderData[index]?.Fuel_amt}</p>
                <p>Fuel Type: {orderData[index]?.Fuel_type}</p>
              </div>
            </div>
            <p className='font-medium'>Phone: {pumpData[index]?.phno}</p>
          </div>
        </div>
      ))}
    </>
